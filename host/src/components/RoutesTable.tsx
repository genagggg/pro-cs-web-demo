...
const initialRoutes: Route[] = [
  { id: '1', route: 'Москва → Санкт-Петербург', date: '2024-04-25', status: 'planned', cargoCount: 8, driver: 'Иванов И.И.' },
  { id: '2', route: 'Казань → Нижний Новгород', date: '2024-04-26', status: 'in_progress', cargoCount: 5, driver: 'Петров П.П.' },
  { id: '3', route: 'Екатеринбург → Челябинск', date: '2024-04-24', status: 'completed', cargoCount: 12, driver: 'Сидоров С.С.' },
  { id: '4', route: 'Ростов-на-Дону → Краснодар', date: '2024-04-27', status: 'planned', cargoCount: 6, driver: 'Кузнецов К.К.' },
  { id: '5', route: 'Новосибирск → Омск', date: '2024-04-28', status: 'in_progress', cargoCount: 9, driver: 'Смирнов С.С.' },
];
    ...
    columnHelper.accessor('status', {
      ...
      cell: (info) => {
        ...
        return isEditing ? (
          <select
            value={status}
            onChange={(e) => updateRouteStatus(info.row.original.id, e.target.value as Route['status'])}
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          <div>
            ...
            <span>
              📅 Ближайшая дата: {new Date(Math.min(...routes.map(r => new Date(r.date).getTime()))).toLocaleDateString('ru-RU')}
            </span>
          </div>
          <div style={{ 
            display: 'flex',
